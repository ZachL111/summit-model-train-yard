...
`summit-model-train-yard` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for train workflows, centered on state machine modeling, transition tables, and invalid-transition tests.
...
The score is deterministic. The current threshold is 173. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 4.
